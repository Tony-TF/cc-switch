...
const TOOL_VERSION_COMMAND_TIMEOUT: Duration = Duration::from_secs(4);
const TOOL_LATEST_VERSION_TIMEOUT: Duration = Duration::from_secs(5);
...
async fn fetch_npm_latest_version(client: &reqwest::Client, package: &str) -> Option<String> {
    ...
    match client
        .get(&url)
        .timeout(TOOL_LATEST_VERSION_TIMEOUT)
        .send()
        .await
    {
        Ok(resp) => {
...
fn try_get_version(tool: &str) -> (Option<String>, Option<String>) {
    ...
    let output = {
        ...
        run_command_with_timeout(command, TOOL_VERSION_COMMAND_TIMEOUT)
    };
...
fn try_get_version_wsl(
    ...
    match output {
        ...
        Err(e) => (None, Some(format!("[WSL:{distro}] {e}"))),
    }
}
